...
async def load_models():
    ...
    global dataset_stats_cache, dashboard_cache
...
def _get_dataset_df() -> pd.DataFrame:
    if rag_system is not None and getattr(rag_system, "df", None) is not None:
        return rag_system.df
    return pd.read_csv(CLEAN_TICKETS_FILE, low_memory=False)


def _compute_dataset_stats() -> Dict:
    df = _get_dataset_df()
    total_tickets = int(len(df))
    categories_count = int(df[TARGET_COLUMN].nunique()) if TARGET_COLUMN in df.columns else 0
    resolved_tickets = int(df["Resolution Comments"].notna().sum()) if "Resolution Comments" in df.columns else 0
...
def _compute_dashboard_data() -> Dict:
    ...
    if "day_of_week" in df.columns:
        ...
        incidents_over_time = [
            {"name": day_names[int(day)], "value": int(count)}
    ...
    if TARGET_COLUMN in df.columns:
        top = df[TARGET_COLUMN].value_counts().head(6)
        total = float(top.sum()) if len(top) else 1.0
        palette = ["#3EB489", "#C7B3FF", "#FFB7A7", "#FFE38A", "#9AD0F5", "#A7F0BA"]
        for (idx, (name, value)) in enumerate(top.items()):
            category_distribution.append(
                {
                    "name": str(name),
                    "value": int(round((float(value) / total) * 100)),
                    "color": palette[idx % len(palette)]
                }
            )

    resolved = int(df["Resolution Comments"].notna().sum()) if "Resolution Comments" in df.columns else 0
    kpis = [
        {"label": "Total Tickets", "value": int(len(df))},
        {"label": "Resolved Tickets", "value": resolved},
        {"label": "Service Categories", "value": int(df[TARGET_COLUMN].nunique()) if TARGET_COLUMN in df.columns else 0}
    ]
...
async def stats():
    ...
    if dataset_stats_cache is None:
        dataset_stats_cache = _compute_dataset_stats()
...
async def dashboard():
    ...
    if dashboard_cache is None:
        dashboard_cache = _compute_dashboard_data()
